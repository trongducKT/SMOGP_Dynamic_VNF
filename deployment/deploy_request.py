...
def deploy(network: Network, request: Request, indi : Individual, vnf_list):
    ...
    for VNF in request.VNFs:
        ...
        for server in vnf_list[VNF].d_f.keys():
            ...
            while path_delay == None or (path_delay > int(T) + n - T):
                n = n + 1
                path_delay, path = dijkstra(network_copy, start_node, server, bw, int(T), int(T) + n, 
                                            network_copy.get_node )
                if n >= int(lifetime - T):
                    break
            ...
            if (state_server["cpu"] < vnf_list[VNF].c_f) or (state_server["mem"] < vnf_list[VNF].h_f) or (state_server["ram"]< vnf_list[VNF].r_f) or (time_to_server + finished_server > request.lifetime):
                ...
                server_chosing.append((server, value_of_gp, path, time_to_server + finished_server, 
                                       cost, T1, T2, T3, T4))
        ...
        if len(server_chosing_sorted) == 0 or server_chosing_sorted[0][1] == -np.inf:
            ...
            update_deploy.append((server_chosing_sorted[0][0], server_chosing_sorted[0][5], 
                                  server_chosing_sorted[0][6], used, server_chosing_sorted[0][2], 
                                  bw, server_chosing_sorted[0][7], server_chosing_sorted[0][8]))
            start_node = server_chosing_sorted[0][0]
            update_link_state(network_copy.links, server_chosing_sorted[0][2], 
                              bw , server_chosing_sorted[0][5], server_chosing_sorted[0][6])
    ...
    while path_delay == None or (path_delay > int(T) +n - T):
        ...
        if n >= int(lifetime - T):
            break
        path_delay, path = dijkstra(network_copy, start_node, end_node, bw, int(T), int(T) + n, 
                                    network_copy.get_node)
        if n >= int(lifetime - T):
            break
