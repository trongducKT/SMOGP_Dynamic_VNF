...
def calFitness_three_policies(indi: Individual, network, request_list, vnf_list):
    ...
    while len(request_list_copy) > 0:
        ...
        for request in request_processing:
            ...
            if value_of_determining_gp < 0:
                reject = reject + 1
                continue
            value_of_ordering_gp = ordering_gp(indi, request, T, network_copy, vnf_list)
        ...
        for re_de in request_decision:
            update_state, cost= deploy(network_copy, re_de[0], indi, vnf_list)
            if update_state == False:
                ...
                for update_item in update_state:
                    if update_item[0] == None:
                        update_link_state(network_copy.links, update_item[4], update_item[5], 
                                          update_item[6], update_item[7])
                    else:
